...
(define (prim.write-string-file filename contents state)
 ...
 (box
  (let ((stream (cl:open (haskell-string->string filename)
			   :direction :output 
			   :if-exists :overwrite
...
(define (prim.write-bin-file name contents)
  (let ((stream (cl:open name :direction :output 
			   :if-exists :overwrite
			   :if-does-not-exist :create)))
...
(define (prim.append-bin-file name contents)
 (let ((bin (lisp-read name)))
   (if (and (pair? bin) (eq? (car bin) ':binary))
       (let ((stream (cl:open name :direction :output :if-exists :overwrite)))
